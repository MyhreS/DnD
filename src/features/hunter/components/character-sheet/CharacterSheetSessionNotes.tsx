...
import { useCharacterSheetSessionNotes } from "@/features/hunter/hooks/useCharacterSheetSessionNotes";
...
export function CharacterSheetSessionNotes({ card, readOnly }: { card: HunterCard; readOnly: boolean }) {
  ...
  const { notes, body, setBody, sending, error, submit } = useCharacterSheetSessionNotes(selected?.id ?? null);
  ...
  if (!sessions.length) return <section className="character-sheet-session-empty">
  ...
  return <div className="character-sheet-session-notes">
    <label className="character-sheet-session-picker">
      <span>Session</span>
    ...
    {writable && <form className="character-sheet-session-compose" onSubmit={addNote}>
      <label htmlFor={`character-sheet-session-note-${selected?.id}`}>Add to this session</label>
      <textarea id={`character-sheet-session-note-${selected?.id}`} value={body} maxLength={2000} rows={3} placeholder="What should the table remember?" onChange={(event) => setBody(event.target.value)} />
      <footer><span>{body.length}/2000</span><button type="submit" disabled={!body.trim() || sending}>{sending ? "Saving…" : "Add note"}</button></footer>
    </form>}

    {(sessionsError || error) && <p className="character-sheet-session-error" role="alert">{sessionsError || error}</p>}
    <section className="character-sheet-session-history" aria-label="Notes from selected session" aria-live="polite">
