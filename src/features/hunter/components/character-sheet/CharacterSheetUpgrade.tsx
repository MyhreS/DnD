...
import { CharacterSheetUpgradeChoices, type UpgradeChoiceKind } from "./CharacterSheetUpgradeChoices";
import { CharacterSheetUpgradeFeatPage } from "./CharacterSheetUpgradeFeatPage";
...
export function CharacterSheetUpgrade({ model, onComplete, creating = false }: { model: AppSheetModel; onComplete: () => void; creating?: boolean }) {
  ...
  const pendingChoices = ([
    ...
    needsSetup && automation.background && !automation.bonusComplete ? { stepId: "background-abilities", label: "Background abilities", detail: "Place +2 and +1 on different abilities, or +1 on all three.", status: backgroundRemaining > 0 ? `${backgroundRemaining} point${backgroundRemaining === 1 ? "" : "s"} needed` : "Choose a valid +2/+1 or +1/+1/+1 split", count: Math.max(1, backgroundRemaining) } : null,
  ...
  function saveUpgrade() {
    if (!canSave) return;
    const setupComplete = !!card.name.trim()
      && !!card.classId
      && !!card.backgroundId
      && automation.pointsLeft === 0
      && automation.bonusComplete
      && remaining === 0;
  ...
  >
      ...
      {step.kind === "choice" && step.choice && <CharacterSheetUpgradeChoices kind={step.choice} target={target} />}
      {step.kind === "equipment" && <CreationEquipment model={model} />}
      {step.kind === "feature" && step.feature && <CharacterSheetUpgradeFeatPage feature={step.feature} state={state} />}
...
function CreationName({ model, name }: { model: AppSheetModel; name: string }) {
  return <div className="character-sheet-upgrade-choice-page character-sheet-creation-name"><p>This name will appear on your character sheet and in your party.</p><label className="character-sheet-upgrade-select"><span>Hunter name</span><input value={name} placeholder="Enter a name" onChange={(event) => model.setFields({ name: event.target.value }, { name: event.target.value })} /></label></div>;
}
...
function Review({ changes, creationValues, features, state, remaining, pendingChoices, onResolve, subclass, subclassFeatures }: { changes: Change[]; creationValues: Array<[string, unknown]>; features: UpgradeFeature[]; state: ReturnType<typeof useCharacterAutomation>["state"]; remaining: number; pendingChoices: PendingChoice[]; onResolve: (stepId: string) => void; subclass?: Subclass; subclassFeatures: LevelFeature[] }) {
  ...
  return <div className="character-sheet-upgrade-review">
    <p>{remaining > 0 ? "Complete the required decisions below before saving." : "Everything below will be applied together."}</p>
    {pendingChoices.length > 0 && <section className="character-sheet-upgrade-review-pending" aria-label="Required decisions"><header><small>Still needed</small><b>{remaining} choice{remaining === 1 ? "" : "s"} left</b></header>{pendingChoices.map((choice) => <button key={choice.stepId} type="button" onClick={() => onResolve(choice.stepId)}><span><b>{choice.label}</b><small>{choice.detail}</small></span><strong>Complete</strong></button>)}</section>}
    {creationValues.length > 0 && <div className="character-sheet-upgrade-review-values">{creationValues.map(([label, value]) => <span key={label}><small>{label}</small><b>{String(value)}</b></span>)}</div>}
    {changes.length > 0 && <div className="character-sheet-upgrade-review-values">{changes.map(([label, before, after]) => <span key={label}><small>{label}</small><b>{before == null ? String(after ?? "—") : `${String(before)} → ${String(after ?? "—")}`}</b></span>)}</div>}
    {subclass && <section className="character-sheet-upgrade-review-subclass"><small>Subclass selected</small><h4>{subclass.name}</h4><p>{subclass.tagline}</p>{subclassFeatures.map((feature) => <article key={`${feature.level}:${feature.name}`}><span><small>Gained at level {feature.level}</small><b>{feature.name}</b></span><p>{feature.text}</p></article>)}</section>}
    ...
    {!hasSummary && <p className="character-sheet-upgrade-review-empty">No character changes are waiting to be saved.</p>}
  </div>;
}
