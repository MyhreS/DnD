...
export function CharacterSheetArmorDoll({ model }: { model: AppSheetModel }) {
  ...
  return <div className="character-sheet-paper-doll">
    ...
    {EXTRA_SLOTS.map(([subcategory, label]) => {
      ...
      return <div key={subcategory} className={`character-sheet-equip-slot character-sheet-equip-${subcategory.toLowerCase().replaceAll(" ", "-")}`}><CharacterSheetEquipmentSocket label={label} name={current?.name} detail={current ? `${current.weightLb} lb` : undefined} kind="armor" disabled={model.readOnly} compact onClick={() => picker.openPicker({
        title: label,
    ...
    <div className="character-sheet-equip-slot character-sheet-equip-main"><CharacterSheetEquipmentSocket label="Main armor" name={mainOptions.find((entry) => entry.id === card.mainArmorId)?.name} detail={mainOptions.find((entry) => entry.id === card.mainArmorId)?.ac ?? "Unarmored"} kind="armor" disabled={model.readOnly} compact onClick={openMain} /></div>
  </div>;
}
