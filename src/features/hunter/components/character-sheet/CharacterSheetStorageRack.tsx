...
import { useEquipmentPicker } from "./characterSheetEquipmentPickerContext";
...
export function CharacterSheetStorageRack({ storageId, units, readOnly }: { storageId: string; units: CarryUnit[]; readOnly: boolean }) {
  ...
  return <details className="character-sheet-storage-rack" open>
    ...
    <div className={`character-sheet-storage-cells${definition.gives.count > 8 ? " is-dense" : ""}`}>
      {slots.map(({ target, unit }, index) => <div className={`character-sheet-storage-cell${unit ? " is-filled" : ""}`} key={target}>
        <small>{index + 1}</small>
        <CharacterSheetEquipmentSocket
          label={`Slot ${index + 1}`}
