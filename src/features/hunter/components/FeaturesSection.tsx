      ...
      {featNames.length > 0 && (
        <div className="stack" style={{ gap: 4, marginBottom: features.length ? 10 : 0 }}>
          {featNames.map((name, i) => {
            ...
            // The origin Skilled feat's chosen proficiencies (skills/tools).
            const isOrigin = i === 0 && card.feat === name;
            const chosen = isOrigin && bare === "Skilled" ? card.featSkills ?? [] : [];
            return (
              <details key={`${name}-${i}`}>
                <summary style={{ cursor: "pointer", fontSize: "0.92rem" }}>
                  <span style={{ fontWeight: 600 }}>{name}</span>
                  <span className="faint" style={{ fontSize: "0.74rem" }}>
                    {" "}· {isOrigin ? "origin feat" : "feat"}
                  </span>
                </summary>
                {text && (
                ...
                {chosen.length > 0 && (
                  <p className="muted" style={{ fontSize: "0.86rem", margin: "6px 0 4px" }}>
                    <span className="gold">Chosen:</span> {chosen.join(", ")}
                  </p>
                )}
              </details>
            );
