...
export function View4CharacterSheet({ model, notesModel, panel, onPanelChange }: { model: AppSheetModel; notesModel: AppSheetModel; panel: View4Panel | null; onPanelChange: (panel: View4Panel | null) => void }) {
  ...
  >
    <header className="v4-identity">
      ...
      <div><button type="button" onClick={() => onPanelChange("resources")}><small>Level</small><strong>{card.level}</strong></button><button type="button" onClick={() => onPanelChange("resources")}><small>Insight</small><strong>{insight}</strong></button></div>
    </header>
    ...
    <section className="v4-vitals" aria-label="Current resources">
      ...
      <button type="button" onClick={() => onPanelChange("resources")}><span><b>Sanity</b><em>{sanity} / {sanityMax}</em></span><i><span style={{ width: `${sanityMax ? Math.max(0, Math.min(100, sanity / sanityMax * 100)) : 0}%` }} /></i></button>
    </section>
