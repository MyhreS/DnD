...
export function View4UpgradeChoices({ kind, target }: { kind: UpgradeChoiceKind; target: number }) {
  ...
  if (kind === "subclass" && klass) {
    ...
    return <div className="v4-upgrade-choice-page"><label className="v4-upgrade-select"><span>{klass.name} path</span><select value={card.subclassId ?? ""} onChange={(event) => automation.chooseSubclass(event.target.value)}><option value="">Choose...</option>{klass.subclasses.map((entry) => <option key={entry.id} value={entry.id}>{entry.name}</option>)}</select></label>{selected && <article className="v4-upgrade-detail"><b>{selected.tagline}</b><p>{selected.blurb}</p><div className="v4-upgrade-subclass-features">{selected.features.map((feature) => <span key={`${feature.level}:${feature.name}`}><small>Level {feature.level}</small><b>{feature.name}</b><p>{feature.text}</p></span>)}</div></article>}</div>;
  }
