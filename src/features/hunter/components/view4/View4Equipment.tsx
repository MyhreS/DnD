...
export function View4Equipment({ model }: { model: AppSheetModel }) {
  ...
  return <div className="v4-equipment">
    ...
    <div className="v4-paper-doll">
      ...
      <label className="v4-equip-slot v4-equip-main"><span>Main armor</span><select value={card.mainArmorId ?? ""} disabled={model.readOnly} onChange={(event) => automation.chooseMainArmor(event.target.value)}><option value="">Unarmored</option>{mainOptions.map((entry) => <option key={entry.id} value={entry.id}>{entry.name} · {entry.ac}</option>)}</select></label>
    </div>
    <section className="v4-addon-slots"><header><h3>Add-on armor</h3><small>Choose up to {addonLimit} pieces</small></header><div>{Array.from({ length: addonLimit }, (_, index) => <label key={index}><span>Slot {index + 1}</span><select value={card.addonArmorIds?.[index] ?? ""} disabled={model.readOnly} onChange={(event) => automation.setAddonArmorAt(index, event.target.value)}><option value="">Empty</option>{addonOptions.map((entry) => <option key={entry.id} value={entry.id}>{entry.name} · {entry.ac}</option>)}</select></label>)}</div></section>
  </div>;
}
