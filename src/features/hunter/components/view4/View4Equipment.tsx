...
import { View4EquipmentPickerProvider } from "./View4EquipmentPicker";

export function View4Equipment({ model }: { model: AppSheetModel }) {
  return <View4EquipmentPickerProvider><EquipmentPage model={model} /></View4EquipmentPickerProvider>;
}
...
function EquipmentPage({ model }: { model: AppSheetModel }) {
  ...
  return <div className="v4-equipment">
    ...
    {layer === "armor" ? <><View4ArmorDoll model={model} /><View4AddonArmor model={model} /></> : <View4CarryingGear model={model} />}
  </div>;
}
