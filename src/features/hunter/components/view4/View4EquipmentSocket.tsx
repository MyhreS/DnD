...
function SlotGlyph({ kind }: { kind: EquipmentGlyph }) {
  if (kind === "empty") return <span className="v4-slot-plus">+</span>;
...
export function View4EquipmentSocket({
  label,
  name,
  detail,
  kind = "empty",
  ...
  return <button
    type="button"
    className={`v4-equipment-socket${empty ? " is-empty" : " is-filled"}${compact ? " is-compact" : ""}`}
    disabled={disabled}
    onClick={onClick}
  >
    <span className="v4-slot-glyph"><SlotGlyph kind={empty ? "empty" : kind} /></span>
    <span className="v4-slot-copy">
      {label && <small>{label}</small>}
      <strong>{name ?? "Add item"}</strong>
      {detail && <em>{detail}</em>}
    </span>
    <span className="v4-slot-chevron" aria-hidden="true">›</span>
  </button>;
}
