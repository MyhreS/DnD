...
export function View4CarryPoint({ location, units, readOnly }: { location: SlotLocation; units: CarryUnit[]; readOnly: boolean }) {
  ...
  return <div className={`v4-carry-point v4-carry-${location}`}>
    ...
    {storageId && <View4EquipmentSocket name={ITEM_BY_ID[storageId]?.name ?? storageId} detail={`Opens ${storage?.gives.count ?? 0} slots`} kind="storage" disabled={readOnly} compact onClick={() => openSlot(1, undefined, storageId)} />}
    {placed.map((unit, index) => <View4EquipmentSocket key={`${unit.item.id}-${unit.index}`} name={unitLabel(unit, units)} detail={`${unit.item.carry} · ${unit.item.weightLb} lb`} kind={glyph(unit)} disabled={readOnly} compact onClick={() => openSlot(index + 1, unit)} />)}
    {canChoose && Array.from({ length: Math.max(1, baseOpen) }, (_, index) => <View4EquipmentSocket key={index} label={`Slot ${placed.length + index + 1}`} disabled={readOnly} compact onClick={() => openSlot(placed.length + index + 1)} />)}
  </div>;
}
